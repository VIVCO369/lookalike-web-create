    ...
    <div className="min-h-screen bg-background flex w-full"> {/* Changed inline style to Tailwind class */}
      ...
      >
        <div className="p-6">
          <AnimatedContainer>
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">$20 To $100 in 30 Days</h1> {/* Added dark mode text color */}
              <p className="text-gray-600 dark:text-gray-400">Track your trading target progress from $20 to $100.</p> {/* Added dark mode text color */}
            </div>
          </AnimatedContainer>
                  ...
                  <tbody>
                    {targetData.map((row, index) => (
                      <tr key={index} className={getRowClass(index, row.week)}>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(0, row.week))}> {/* Added dark mode border */}
                          {row.week}
                        </td>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(1, row.days))}> {/* Added dark mode border */}
                          {row.days}
                        </td>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(2, row.amount))}> {/* Added dark mode border */}
                          {typeof row.amount === "number" ? `$${row.amount.toFixed(2)}` : row.amount}
                        </td>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(3, row.dailyProfit))}> {/* Added dark mode border */}
                          {row.dailyProfit}
                        </td>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(4, row.lotSize))}> {/* Added dark mode border */}
                          {typeof row.lotSize === "number" ? `$${row.lotSize.toFixed(2)}` : row.lotSize}
                        </td>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(5, row.amountTrade))}> {/* Added dark mode border */}
                          {typeof row.amountTrade === "number" ? row.amountTrade.toFixed(2) : row.amountTrade}
                        </td>
                        <td className={cn(`border border-gray-400 dark:border-gray-600 px-3 py-2 text-center`, getCellClass(6, row.trades))}> {/* Added dark mode border */}
                          {row.trades}
                        </td>
                      </tr>
                    ))}
